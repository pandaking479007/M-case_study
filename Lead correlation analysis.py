...
file_path = r'C:\Users\lanji\OneDrive\Desktop\Cleaned - Updated Leads - Sheet1.csv'

# List of columns to exclude
columns_to_exclude = ['LeadId', 'Title', 'Industry','Industry_cleaned' ,'Employees', 'CreatedDate', 'LeadSource','OwnerEmployeeId', 'OwnerEmployee',
                      'ConvertedOpportunityId', 'TimeZone', 'Date Key', 'Date', 'opps amount','opps converted','DisqualifiedReason']
...
for i in range(len(corr_matrix.columns)):
    for j in range(len(corr_matrix.columns)):
        if np.abs(corr_matrix.iloc[i, j]) >= 0.2:
...
plt.title('Lead Correlation Analysis', fontsize=20)
